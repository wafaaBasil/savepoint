...
class UserController extends BaseController
{
    public function index(Request $request)
    {
        if($request->page == null){
            $users = User::where('provider_id',auth("sanctum")->user()->provider_id)->orderBy('created_at','desc')->get();
            $page_count = null;
        }else{
            $users = User::where('provider_id',auth("sanctum")->user()->provider_id)->orderBy('created_at','desc')->paginate(10);
